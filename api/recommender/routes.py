...
def get_popular_items(kitchen_id):
	...
	try:
		...
		response = recommender_engine.get_popular_items(str(kitchen_id), int(N))
	except NoClass as e:
	...
	end_time = time.time()
	logger('NUCLEUS_RECOMMENDER', 'EXE_TIME', 'Execution time of start_time() for kitchen_id={} with N={} is {}.'.format(kitchen_id, N, end_time-start_time))
